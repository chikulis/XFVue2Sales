          ...
          <el-row :gutter="20">
            ...
            <el-col :span="6">
              ...
              >
                <el-input
                  disabled
                  v-model="addFormData.companyname"
                ></el-input>
              </el-form-item>
            </el-col>
...
export default {
  ...
  created() {
    ...
    if (this.$route.params.multipleSelection) {
      ...
      // var dateNow=,
      this.addFormData.DocDate=this.$moment()
          .subtract("days", 0)
          .format("YYYY-MM-DD"),
      this.addFormData.DelDate=this.$moment()
          .subtract("days", 0)
          .format("YYYY-MM-DD"),
      this.addFormData.AlPriceDate=this.$moment()
          .subtract("days", 0)
          .format("YYYY-MM-DD"),
      ...
      this.addFormData.EnterDate=this.$moment()
          .subtract("days", 0)
          .format("YYYY-MM-DD"),
  ...
  methods: {
    ...
    saveRowEvent(row) {
      ...
      if (row.type == "update") {
        this.$api.slssalesorderitem.update(row.row).then(res => {
      ...
      return;
        ...
        {
          this.$api.slssalesorderitem.add(this.addFormData.doccode,row.row).then(res => {
    ...
    okTableData() {
      this.$api.slssalesorderhd
        .conifrmdoc(
          this.addFormData.doccode,
          1,
          this.addFormData.hrcode,
          ''
        )
        .then(res => {
          if (res.data.code == 200) {
            ...
            this.$alert("确认失败:" + JSON.parse(res.data.message)[0].Memo==null?res.data.message:JSON.parse(res.data.message)[0].Memo, "错误提示", {
    ...
    savTableData()
    {
      this.$api.slssalesorderhd.save(this.addFormData).then((res) => {
                // if (res.status == 201) {
